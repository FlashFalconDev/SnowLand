...
def sha256_hash(encrypt_data, key, iv):
    # TradeValue = "HashIV=" + iv + "&" + encrypt_data + "&" + "HashKey=" + key
    # TradeValue = "HashKey=" + key + "&" + encrypt_data + "&" + "HashIV=" + iv
    TradeValue = f"HashKey={key}&{encrypt_data}&HashIV={iv}"
    hashs = hashlib.sha256(TradeValue.encode("utf-8")).hexdigest()
    hashs = str.upper(hashs)
    return hashs


def neweb_pay_request(data):
